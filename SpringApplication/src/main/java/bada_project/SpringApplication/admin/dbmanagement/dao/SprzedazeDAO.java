...
public class SprzedazeDAO {
    ...
    public Optional<Sprzedaze> findById(Long id) {
        ...
        List<Sprzedaze> list = jdbc.query(sql, rowMapper(), id);
    ...
    public void insert(Sprzedaze sprzedaz) {
        ...
        jdbc.update(
                ...
                new java.sql.Date(sprzedaz.getDataSprzedazy().getTime())
        );
    }
    ...
    public void delete(Long id) {
        jdbc.update("DELETE FROM SPRZEDAZE WHERE id_sprzedazy = ?", id);
    }

    private RowMapper<Sprzedaze> rowMapper() {
        return (rs, rowNum) -> {
            Sprzedaze s = new Sprzedaze();

            // Core fields - these are always present
            ...
            // Display fields from joins - may be null
